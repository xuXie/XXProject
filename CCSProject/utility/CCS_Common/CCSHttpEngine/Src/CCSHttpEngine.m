...
//  Copyright 2011年 Tian Yu CO.,LTD. All rights reserved.
//
...
//        _m_timeInterval_timeout = 10;              // 默认超时20秒
...
//    UIAlertView *l_alert=[[UIAlertView alloc] initWithTitle:@"测试" message:[NSString stringWithFormat:@"%@",resultUrl] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
//    [l_alert show];
...
- (void)requestFinished:(ASIHTTPRequest *)request{
    NSData *l_data_response = [request responseData];
    
    NSString *l_str_response = [[NSString alloc]initWithBytes:[l_data_response bytes] length:[l_data_response length] encoding:NSUTF8StringEncoding];
    ...
    NSLog(@"\n[SYS] Request totalBytesSent : %lld bytes, %.2lf kb \nRequest totalBytesRead : %lld bytes, %.2lf kb \nSuccess responseString: %@", 
          ...
          [request totalBytesRead], [request totalBytesRead]/1024.0, l_str_response);
    id object = [[request userInfo] objectForKey:@"object"];
    SEL finishedAction=nil;
