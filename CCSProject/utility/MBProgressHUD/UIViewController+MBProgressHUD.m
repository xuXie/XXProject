...
- (MBProgressHUD *)progressHUD
{
    ...
    if(!hud)
    {
        ...
        hudSuperView=[[UIApplication sharedApplication] keyWindow];
...
{
    self.progressHUD.customView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"success"]] autorelease];
...
{
    self.progressHUD.customView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"error"]] autorelease];
	self.progressHUD.mode = MBProgressHUDModeCustomView;
    self.progressHUD.labelText = message;
    [self performSelector:@selector(hideHUD) withObject:nil afterDelay:1];
}
